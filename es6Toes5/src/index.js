...
function generateMonthPicker(offsetPositionArr, offsetHeight) {
	...
	//拼接日历组件内容
	monthPickerWrapEl =
		'<div class="x-month-picker-wrapper">' +
		'	<div class="x-month-picker-header">' +
		'		<div class="x-month-prev-year common-toggle-date-btn">' +
		...
		"		</div>" +
		'		<div class="x-month-year-wrap">' +
		'			<span class="x-month-year-txt"></span>' +
		"			<span>年</span>" +
		"		</div>" +
		'		<div class="x-month-next-year common-toggle-date-btn">' +
		...
		"		</div>" +
		"	</div>" +
		'	<div class="x-month-picker-body">' +
		generateMonthPickerItem() +
		"	</div>" +
		"</div>   ";
	...
	function generateMonthPickerItem() {
		...
		for (let i = 1; i <= 12; i++) {
			...
			monthPickerItemStr +=
				'<div class="x-month-picker-item" data-date="' +
				__monthStr +
				'"><span>' +
				monthIdxMap[i - 1] +
				"</span></div>";
...
function handleClickCheck() {
	$(document)
		...
		.click(function (e) {
			...
			if (
				e.target === monthPicker__this ||
				monthPickerWrapEl.get(0).contains(e.target)
			) {
				console.log();
			} else {
...
function handleDayClick(curThis) {
	$(".x-month-picker-body")
		.off()
		.on("click", ".x-month-picker-item>span", function () {
			var __dataMonth = $(this)
				.parents(".x-month-picker-item")
				.attr("data-date");
